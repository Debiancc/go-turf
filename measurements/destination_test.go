...
func TestDestination(t *testing.T) {
	...
	p1 := Destination(*features.NewPoint([2]float64{-75.00, 38.10096062273525}), 100, 0, types.UnitKilometers, nil)
	assert.Equal(p1.Geometry.Coordinates[1], 39.00028098645979)
	assert.Equal(p1.Geometry.Coordinates[0], -75.00000000000001)

	p2 := Destination(*features.NewPoint([2]float64{-75.00, 39}), 100, 90, types.UnitKilometers, nil)
	assert.Equal(p2.Geometry.Coordinates[1], 38.99428496385679)
	assert.Equal(p2.Geometry.Coordinates[0], -73.84285322793312)

	p3 := Destination(*features.NewPoint([2]float64{-75.00, 39}), 100, 180, types.UnitKilometers, nil)
	assert.Equal(p3.Geometry.Coordinates[1], 38.10067963627546)
	assert.Equal(p3.Geometry.Coordinates[0], -75.00000000000001)

	p4 := Destination(*features.NewPoint([2]float64{-75, 39}), 5000, 90, types.UnitKilometers, nil)
